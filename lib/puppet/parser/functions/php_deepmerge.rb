module Puppet::Parser::Functions
  newfunction(:php_deepmerge, :type => :rvalue, :doc => <<-'ENDHEREDOC') do |args|
    ...
    For example:
        ...
        $merged_hash = php_deepmerge($hash1, $hash2)
    ...
    if args.length < 2
      raise Puppet::ParseError, ("php_deepmerge(): wrong number of arguments (#{args.length}; must be at least 2)")
    end
    ...
    args.each do |arg|
      ...
      unless arg.is_a?(Hash)
        raise Puppet::ParseError, "php_deepmerge: unexpected argument type #{arg.class}, only expects hash arguments"
      end
